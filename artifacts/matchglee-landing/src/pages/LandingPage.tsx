  ...
  return (
    <section ref={ref as React.RefObject<HTMLElement>} id="ch-1" className="relative w-full h-screen overflow-hidden flex flex-col justify-center items-center p-6 bg-black" style={{ scrollSnapAlign: "start" }}>
      <div className={`absolute inset-0 flex items-center justify-center gap-2 transition-opacity duration-1000 ${isVisible ? "opacity-10" : "opacity-0"}`}>
        {[...Array(20)].map((_, i) => (
          <div key={i} className="w-1 bg-white rounded-full" style={{ height: `${(i * 17 + 23) % 60 + 10}vh`, animation: `pulse ${(i % 3) + 1}s infinite`, animationDelay: `${(i * 0.1) % 1}s` }} />
        ))}
      </div>
      ...
      <div ref={vRef} className="max-w-7xl mx-auto w-full flex flex-col gap-8">
        <div className="text-center flex flex-col items-center gap-4">
          <div className={`transition-all duration-700 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-10"}`}>
            <img src={`${BASE}images/matchglee-logo.jpeg`} alt="MatchGlee" className="w-14 h-14 rounded-2xl object-cover mx-auto shadow-[0_0_40px_rgba(196,181,253,0.5)]" />
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          ...
          <div className={`relative flex justify-center items-center transition-all duration-1000 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-16"}`} style={{ transitionDelay: "900ms" }}>
            ...
            <div className={`absolute -right-4 top-16 w-48 p-4 ${pearlCard} z-20 hidden md:block`} style={{ animation: "floatBounce 4s ease-in-out infinite" }}>
              <div className="flex items-center gap-3">
            ...
            <div className={`absolute -left-10 bottom-20 w-52 p-4 ${pearlCard} z-20 hidden md:block`} style={{ animation: "floatBounce 5s ease-in-out 0.5s infinite" }}>
...
function Chapter5({ onVisible }: { onVisible: (id: string) => void }) {
  ...
  return (
    <section ref={ref as React.RefObject<HTMLElement>} id="ch-5" className="relative w-full h-screen overflow-hidden flex flex-col justify-center items-start p-6 md:p-20 bg-[#080818]" style={{ scrollSnapAlign: "start" }}>
...
function Chapter6({ onVisible }: { onVisible: (id: string) => void }) {
  ...
  return (
    <section ref={ref as React.RefObject<HTMLElement>} id="ch-6" className="relative w-full h-screen overflow-hidden flex flex-col justify-center items-center p-6 bg-[#0D0122]" style={{ scrollSnapAlign: "start" }}>
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {profiles.map((p, i) => (
          <div key={i}
            className={`absolute p-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md transition-all duration-1000 ${isVisible ? "opacity-30 scale-100" : "opacity-0 scale-50"}`}
            ...
            <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${p.color} mb-2`} />
            <div className="h-2 w-16 bg-white/20 rounded mb-1" />
            <div className="h-2 w-10 bg-white/10 rounded" />
          </div>
        ))}
      </div>
      <div ref={vRef} className="relative z-10 text-center max-w-4xl mx-auto">
        <h2 className={`text-5xl md:text-7xl font-bold text-white mb-8 transition-all duration-1000 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
        ...
        <div className={`mt-10 flex justify-center relative z-10 transition-all duration-1000 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-20"}`} style={{ transitionDelay: "600ms" }}>
          <div className="w-[180px] md:w-[240px] rounded-[2rem] p-2 bg-gradient-to-tr from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rotate-[-5deg] shadow-[0_30px_80px_-10px_rgba(139,92,246,0.4)] hover:rotate-0 transition-transform duration-700 ease-out">
            <div className="rounded-[1.5rem] overflow-hidden bg-black">
              <img src={`${BASE}images/matchglee-app-ui.png`} alt="MatchGlee App" className="w-full h-auto opacity-75" />
            </div>
          </div>
        </div>
      </div>
...
function Chapter7({ onVisible }: { onVisible: (id: string) => void }) {
  ...
  return (
    <section ref={ref as React.RefObject<HTMLElement>} id="ch-7" className="relative w-full h-screen overflow-hidden flex flex-col justify-center items-center p-6 bg-[#0A0118]" style={{ scrollSnapAlign: "start" }}>
      <div className={`absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-purple-900/30 via-[#0A0118] to-[#0A0118] transition-opacity duration-[2000ms] ${isVisible ? "opacity-100" : "opacity-0"}`} />
      <div ref={vRef} className="z-10 text-center max-w-2xl w-full flex flex-col items-center">
        ...
        <div className={`mt-20 flex flex-col items-center gap-3 transition-all duration-1000 ease-out ${isVisible ? "opacity-100" : "opacity-0"}`} style={{ transitionDelay: "600ms" }}>
          <img src={`${BASE}images/matchglee-logo.jpeg`} alt="MatchGlee" className="w-10 h-10 rounded-xl object-cover grayscale opacity-40" />
...
function CinematicPhase() {
  ...
  const chapterIds = ["ch-1", "ch-2", "ch-3", "ch-4", "ch-5", "ch-6", "ch-7"];
        ...
        {chapterIds.map((id, i) => {
          ...
          return (
            <button key={id} onClick={() => scrollTo(id)} className="group flex items-center justify-end gap-2" aria-label={`Chapter ${i + 1}`}>
              ...
              <Circle className={`w-3 h-3 transition-all duration-300 ${isActive ? `scale-150 ${isPearl ? "fill-[#C4B5FD] text-[#C4B5FD]" : "fill-pink-500 text-pink-500"}` : `${isPearl && activeChapter.match(/ch-[34]/) ? "text-[#C4B5FD]/50" : "text-white/30"} hover:scale-125`}`} />
            </button>
          );
        })}
      </div>
